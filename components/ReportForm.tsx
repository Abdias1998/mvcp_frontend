...
import { Report, InvitedPerson, Visit, Cell, Group, District, PrayerRequest } from '../types.ts';
import { api } from '../services/api.real';
import { PlusCircleIcon, TrashIcon, SpinnerIcon, HeartIcon } from './icons.tsx';
...
const STEPS = [
  ...
  { id: 4, title: 'Témoignages & Prières' },
...
const ReportForm: React.FC = () => {
    ...
    useEffect(() => {
        const fetchHierarchyData = async () => {
            try {
                // Charger les groupes et districts (pas besoin d'authentification)
                ...
                // Charger les cellules si l'utilisateur est connecté
                if (user) {
                    const cells = await api.getCellsForUser(user);
                    setAllCells(cells);
                } else {
                    // Si pas d'utilisateur, charger toutes les cellules via l'endpoint sans filtre
                    const cells = await api.getCellsForUser({ role: 'NATIONAL_COORDINATOR' } as any);
                    setAllCells(cells);
                }
            } catch (error) {
    ...
    // Handlers for Prayer Requests
    const handlePrayerRequestChange = (index: number, field: keyof Omit<PrayerRequest, 'id'>, value: string | boolean) => {
        const newRequests = [...(formData.prayerRequests || [])];
        newRequests[index] = { ...newRequests[index], [field]: value };
        setFormData(prev => ({ ...prev, prayerRequests: newRequests }));
    };
    const addPrayerRequest = () => setFormData(prev => ({ ...prev, prayerRequests: [...(prev.prayerRequests || []), { id: `prayer-${Date.now()}`, subject: '', isAnonymous: false }] }));
    const removePrayerRequest = (index: number) => setFormData(prev => ({ ...prev, prayerRequests: (prev.prayerRequests || []).filter((_, i) => i !== index) }));
    ...
    const validateStep = (stepToValidate: number, redirectOnError: boolean = false): boolean => {
      if (stepToValidate === 1) {
        ...
        // if (!cellName) {
        //     showToast("Veuillez sélectionner une cellule existante ou saisir le nom d'une nouvelle cellule.", 'error');
        //     if (redirectOnError) setStep(1);
        //     return false;
        // }
      }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            console.log('📝 Soumission du rapport:', reportToSubmit);
            const submittedReport = await api.submitReport(reportToSubmit);
            console.log('✅ Rapport soumis avec succès:', submittedReport);
        ...
        <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                 ...
                 {/* <div>
                    <label htmlFor="cellId" className={labelClass}>Sélectionner une cellule existante (optionnel)</label>
                     <select id="cellId" name="cellId" value={cellsInDistrict.find(c => c.cellName === formData.cellName)?.id || ''} onChange={handleCellChange} className={inputClass} disabled={!formData.district}>
        ...
        <div className="space-y-6">
            ...
            <div className="p-4 border rounded-lg bg-gray-50">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                    ...
                    <p><strong className="text-gray-600">{groupLabel}:</strong> {formData.group}</p>
                    <p><strong className="text-gray-600">{districtLabel}:</strong> {formData.district}</p>
    ...
    return (
      <div className="bg-white p-8 rounded-xl shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          ...
          {step === 4 && (
            <section className="space-y-6 animate-fade-in">
              <h3 className={sectionTitleClass}>Étape 4: Témoignages & Prières</h3>
              <div>
              ...
              <div className="pt-6 space-y-4">
                  <div className="flex justify-between items-center">
                      <h4 className="text-lg font-semibold text-gray-700 flex items-center"><HeartIcon className="h-5 w-5 mr-2 text-red-500" />Sujets de prière</h4>
                      <button type="button" onClick={addPrayerRequest} className="flex items-center space-x-2 text-sm text-blue-600 hover:text-blue-800 font-medium">
                          <PlusCircleIcon className="h-5 w-5" /> <span>Ajouter</span>
                      </button>
                  </div>
                  {(formData.prayerRequests || []).map((req, index) => (
                      <div key={req.id} className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 border rounded-md relative bg-gray-50">
                          <input type="text" placeholder="Sujet de prière" value={req.subject} onChange={e => handlePrayerRequestChange(index, 'subject', e.target.value)} className={`${inputClass} md:col-span-3`}/>
                          <div className="flex items-center justify-end">
                              <input type="checkbox" id={`isAnonymous-${index}`} checked={req.isAnonymous} onChange={e => handlePrayerRequestChange(index, 'isAnonymous', e.target.checked)} className="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
                              <label htmlFor={`isAnonymous-${index}`} className="ml-2 block text-sm text-gray-900">Anonyme</label>
                          </div>
                          <button type="button" onClick={() => removePrayerRequest(index)} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-0.5 hover:bg-red-600">
                              <TrashIcon className="h-4 w-4" />
                          </button>
                      </div>
                  ))}
