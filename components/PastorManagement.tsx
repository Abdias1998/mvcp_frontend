...
const getRoleLabel = (role: UserRole, region?: string) => {
    const isLittoral = region === 'Littoral';
    switch(role) {
        case UserRole.REGIONAL_PASTOR: return "Pasteur Régional";
        case UserRole.GROUP_PASTOR: return isLittoral ? "Pasteur de Groupe" : "Pasteur de District";
        case UserRole.DISTRICT_PASTOR: return isLittoral ? "Pasteur de District" : "Pasteur de Localité";
        default: return "N/A";
    }
}
...
const PastorManagement: React.FC = () => {
    ...
    const handleExport = () => {
        ...
        const dataToExport = filteredPastors.map(p => ({
            Nom: p.name, Email: p.email, "Téléphone": p.contact || '', Rôle: getRoleLabel(p.role, p.region), Région: p.region || '', "Groupe/District": p.group || '', "District/Localité": p.district || ''
        }));
                            ...
                            <tbody>
                                {pendingPastors.map(pastor => (
                                    <tr key={pastor.uid} className="border-b border-yellow-200">
                                        ...
                                        <td className="px-4 py-3 text-xs">{getRoleLabel(pastor.role, pastor.region)}<br/><span>{[pastor.region, pastor.group, pastor.district].filter(Boolean).join(' > ')}</span></td>
            ...
            <div className="bg-white p-6 rounded-xl shadow-md space-y-6">
                ...
                <div className="border-b border-gray-200">
                    <nav className="flex space-x-2" aria-label="Tabs">
                        ...
                        <TabButton tabId={UserRole.GROUP_PASTOR} label="De Groupe / District" />
                        <TabButton tabId={UserRole.DISTRICT_PASTOR} label="De District / Localité" />
                    </nav>
                </div>
                        ...
                        <tbody>
                            {filteredPastors.map(pastor => (
                                <tr key={pastor.uid} className="bg-white border-b hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4">{getRoleLabel(pastor.role, pastor.region)}</td>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {!formData.uid && (
             ...
             <div>
                ...
                <select id="role" name="role" value={formData.role} onChange={handleRoleChange} className={inputClass} required>
                    ...
                    <option value={UserRole.GROUP_PASTOR}>{getRoleLabel(UserRole.GROUP_PASTOR, formData.region)}</option>
                    <option value={UserRole.DISTRICT_PASTOR}>{getRoleLabel(UserRole.DISTRICT_PASTOR, formData.region)}</option>
                </select>
            </div>
            ...
            {(formData.role === UserRole.GROUP_PASTOR || formData.role === UserRole.DISTRICT_PASTOR) && (
                 <div>
                    ...
                    <input type="text" id="group" name="group" value={formData.group || ''} onChange={handleChange} className={inputClass} placeholder={`Nom du ${groupLabel.toLowerCase()}`} required />
                </div>
            )}
             ...
             {formData.role === UserRole.DISTRICT_PASTOR && (
                 <div>
                    ...
                    <input type="text" id="district" name="district" value={formData.district || ''} onChange={handleChange} className={inputClass} placeholder={`Nom de la ${districtLabel.toLowerCase()}`} required />
                </div>
            )}
