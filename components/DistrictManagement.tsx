...
// Composant pour la table paginée des districts
const PaginatedDistrictsTable: React.FC<{
    districts: District[];
    handleEdit: (district: District) => void;
    handleDeleteRequest: (id: string, name: string) => void;
}> = ({ districts, handleEdit, handleDeleteRequest }) => {
    const pagination = usePagination<District>({ items: districts, itemsPerPage: 10 });
    
    if (districts.length === 0) {
        return <p className="text-center text-gray-500 py-6">Aucun district trouvé.</p>;
    }
        ...
        <>
            <table className="w-full text-sm text-left text-gray-600">
                <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                    <tr>
                        <th className="px-6 py-3">Nom du District</th>
...
const DistrictForm: React.FC<{ district: Partial<District>, onSave: (data: Omit<District, 'id'>, districtId?: string) => void, onCancel: () => void }> = ({ district, onSave, onCancel }) => {
    ...
    const nameLabel = isLittoral ? 'Nom du District' : 'Nom de la Localité';
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
                <label htmlFor="name" className={labelClass}>{nameLabel}</label>
                <input type="text" id="name" name="name" value={formData.name || ''} onChange={handleChange} className={inputClass} placeholder={nameLabel} required />
            </div>
