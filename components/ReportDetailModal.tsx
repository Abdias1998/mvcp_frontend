                ...
                <div className="p-6 space-y-6 overflow-y-auto">
                    ...
                    <div className="border-t border-gray-200">
                        <dl className="divide-y divide-gray-200">
                           <DetailRow label="Région" value={report.region} />
                           <DetailRow label={isLittoral ? 'Groupe' : 'District'} value={report.group} />
                           <DetailRow label={isLittoral ? 'District' : 'Localité'} value={report.district} />
                    ...
                    {report.prayerRequests && report.prayerRequests.length > 0 && (
                        <div>
                            <h4 className="font-semibold text-gray-700 mb-2">Sujets de Prière ({report.prayerRequests.length})</h4>
                            <ul className="border border-gray-200 rounded-md divide-y divide-gray-200">
                                {report.prayerRequests.map((p, i) => (
                                    <li key={i} className="pl-3 pr-4 py-3 flex items-center justify-between text-sm">
                                        <div className="w-0 flex-1 flex items-center">
                                            <span className="ml-2 flex-1 w-0">
                                                {p.subject} {p.isAnonymous && <span className="text-xs text-gray-500 italic ml-2">(Anonyme)</span>}
                                            </span>
                                        </div>
                                    </li>
                                ))}
